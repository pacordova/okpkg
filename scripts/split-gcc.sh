...
pkgname=$(printf $version | awk -F. '{print "gcc" $1}')
...
if [ -f gcc/usr/bin/gm2 ]; then
    ...
    mv gcc-gm2 "gcc-gm2-$version-x86_64"
    okpkg makepkg "gcc-gm2-$version-x86_64"
    okpkg install "gcc-gm2-$version-x86_64.tar.lz"
fi
...
okpkg install "gcc-libs-$version-x86_64.tar.lz"
...
if [ -f gcc/usr/bin/gnat ]; then
    ...
    okpkg install "gcc-ada-$version-x86_64.tar.lz"
fi
...
if [ -f gcc/usr/bin/gdc ]; then
    ...
    okpkg install "gcc-d-$version-x86_64.tar.lz"
fi
...
if [ -f gcc/usr/lib64/gcc/$target/$version/cc1obj ]; then
    ...
    okpkg install "gcc-objc-"$version"-x86_64.tar.lz"
fi
...
if [ -f gcc/usr/bin/go ]; then
    ...
    okpkg install "gcc-go-$version-x86_64.tar.lz"
fi
...
if [ -f gcc/usr/bin/gfortran ]; then
    ...
    okpkg install "gcc-fortran-$version-x86_64.tar.lz"
fi
...
okpkg install "gcc-$version-x86_64.tar.lz"
rm -f "$pkgname"*.tar.lz
