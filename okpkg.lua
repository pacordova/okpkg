...
local chdir, setenv, unsetenv, basename, mkdir, pwd =
   ok.chdir, ok.setenv, ok.unsetenv, ok.basename, ok.mkdir, ok.pwd

-- global
sha3sum, chroot = ok.sha3sum, ok.chroot
...
setenv("LC_ALL", "POSIX")
setenv("make", "/usr/bin/make -j5")
setenv("ninja", "/usr/bin/samu")
...
local function vstr(path) local f, i, j
   f = basename(path)
   j = f:find("%.tar") or f:find("%.tgz") or f:find("%.deb")
...
function download(pkgname) local t, f, fp
   ...
   -- change mirrors
   t.url = t.url:
      gsub("https://ftp.gnu.org", "http://mirror.fcix.net"):
...
function build(pkgname) local t, v, status, destdir, fp
   ...
   v = vstr(t.url)
   status = true
   destdir = string.format("/usr/okpkg/packages/%s-%s-amd64", pkgname, v)
   ...
   if t.build == "meson" then
      ...
      table.insert(t.flags, 2, "-Dlibexecdir=/usr/lib64")
      table.insert(t.flags, 3, "-Dbuildtype=release")
      table.insert(t.flags, 4, "-Dwrap_mode=nodownload")
      t.flags = table.concat(t.flags, ' ')
      status =
...
function emerge(pkgname) install(build(download(pkgname))) end
